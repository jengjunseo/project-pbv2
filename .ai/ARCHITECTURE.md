...
## Write consistency
Writes use a short coarse Redis lock because there are only 100 slots and write throughput is not the bottleneck. This keeps usage accounting and eviction simple.

Order is stored in a sorted set. A save performs `ZADD(score=updatedAt)`. Eviction reads the set oldest-first and removes slots until logical usage is under the configured soft cap.
