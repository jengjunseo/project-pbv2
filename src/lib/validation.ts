...
export const slotWriteSchema = z.object({
  id: slotIdSchema,
...
export function validateUploadMeta(input: {
  ...
  if (!name || name.length > 180) {
    return { ok: false, message: "파일 이름은 1~180자여야 합니다." };
  }
  ...
  if (input.size > getMaxFileBytes()) {
    return { ok: false, message: "파일이 허용된 최대 크기를 넘었습니다." };
  }

  const extension = getFileExtension(name);
  if (BLOCKED_EXTENSIONS.has(extension)) {
    return { ok: false, message: `.${extension} 파일은 업로드할 수 없습니다.` };
  }

  const mime = input.type.trim().toLowerCase() || "application/octet-stream";
  if (!ALLOWED_MIME_TYPES.has(mime) && !SAFE_SOURCE_EXTENSIONS.has(extension)) {
