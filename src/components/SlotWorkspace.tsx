...
import {
  ArrowLeft, Check, Clipboard, Copy, Download, FilePlus2, RefreshCw,
  Save, Share2, Trash2, X,
} from "lucide-react";
import Link from "next/link";
import {
  type ChangeEvent, type DragEvent, useCallback, useEffect, useMemo, useRef, useState,
} from "react";
...
export function SlotWorkspace({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("/api/slot", {
        ...
        body: JSON.stringify({ id, text: textRef.current, file: uploaded }),
      });
  ...
  async function clear() {
    ...
    if (!response.ok || !data.ok) {
      setState("error");
      setMessage(data.error?.message ?? "비우지 못했습니다.");
      return;
    }
    setSlot(null);
    setText("");
      ...
      <div className="workspace-grid">
        <section className="editor-panel">
          <div className="slot-heading">
            ...
            <div className="heading-actions">
              <button className="ghost-button" onClick={() => void refresh()} disabled={isRefreshing}>
          ...
          <textarea
            ...
            onChange={(event) => { setText(event.target.value); markDirty(); }}
            aria-label="슬롯 텍스트"
          />

          <div className="editor-footer">
            <span>{text.length.toLocaleString()} / 30,000</span>
            <button className="text-action" onClick={() => void copyText()} disabled={!text}>
        ...
        <aside className="side-panel">
          <div className="side-section">
            ...
            {pendingFile ? (
              ...
              <div className="file-card">
                ...
                <a className="icon-button" href={existingFile.url} target="_blank" rel="noreferrer" aria-label="파일 열기"><Download size={17} /></a>
              </div>
            ) : (
              <div
          ...
          <div className="side-actions">
            <button className="primary-button" onClick={() => void save()} disabled={state === "saving" || !isDirty}>
            ...
            <button className="danger-button" onClick={() => void clear()} disabled={!slot && !text && !pendingFile}>
              <Trash2 size={17} /> 비우기
            </button>
          </div>
