...
export async function POST(request: Request) {
  ...
  try {
    const response = await handleUpload({
      body,
      request,
      onBeforeGenerateToken: async (pathname: string, clientPayload: string | null | undefined) => {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: { message: cause instanceof Error ? cause.message : "업로드에 실패했습니다." },
      },
      { status: 400 },
    );
  }
...
export async function DELETE(request: Request) {
  ...
  try {
    payload = (await request.json()) as { slotId?: unknown; pathname?: unknown };
  } catch {
  ...
  try {
    await cleanupOrphanBlob(pathname, slotId);
  } catch {
    // Keep the object if Redis cannot prove it is orphaned.
  }
