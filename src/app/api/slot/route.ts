...
export async function GET(request: Request): Promise<NextResponse<SlotReadResponse | PBErrorResponse>> {
  ...
  try {
    ...
    return NextResponse.json(
      slot ? { ok: true, empty: false, slot } : { ok: true, empty: true, slot: null },
...
export async function POST(request: Request): Promise<NextResponse<SlotWriteResponse | PBErrorResponse>> {
  if (!(await checkWriteRateLimit(request))) {
    return error("RATE_LIMITED", "저장 요청이 너무 많습니다. 잠시 후 다시 시도하세요.", 429);
  }
  ...
  if (file) {
    ...
    if (!validateBlobPath(id, file.pathname) || !validateBlobUrl(file.url)) {
      return error("INVALID_FILE", "파일 출처가 올바르지 않습니다.", 400);
    }

    try {
      const blob = await head(file.url);
      if (blob.pathname !== file.pathname || blob.url !== file.url || blob.size !== file.size) {
        return error("INVALID_FILE", "업로드된 파일 정보가 일치하지 않습니다.", 400);
      }
    } catch {
      return error("INVALID_FILE", "업로드된 파일을 확인할 수 없습니다.", 400);
    }
  }
...
export async function DELETE(request: Request): Promise<NextResponse<{ ok: true; cleared: boolean } | PBErrorResponse>> {
  if (!(await checkWriteRateLimit(request))) {
    return error("RATE_LIMITED", "요청이 너무 많습니다. 잠시 후 다시 시도하세요.", 429);
  }
