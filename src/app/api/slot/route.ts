...
import { parseSlotId, slotWriteSchema, validateBlobPath, validateUploadMeta } from "@/lib/validation";
...
export async function POST(request: Request): Promise<NextResponse<SlotWriteResponse | PBErrorResponse>> {
  ...
  if (file) {
    ...
    if (!validateBlobPath(id, file.pathname)) return error("INVALID_FILE", "파일 경로가 슬롯과 일치하지 않습니다.", 400);
  }
  try {
